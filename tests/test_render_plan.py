...
def test_render_markdown_includes_only_false_scope_exclusive(repo_root: Path) -> None:
    render_module = load_module(
        "render_plan", repo_root / "agent-os" / "scripts" / "render-plan.py"
    )
    markdown = render_module.render_markdown(
    ...
    assert markdown.count("- **Scope exclusive**: `false`") == 1
    assert "- **Scope exclusive**: `true`" not in markdown
