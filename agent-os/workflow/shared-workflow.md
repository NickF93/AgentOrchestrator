...
│   ├── .kilo/rules/governance.md   <- repo-local Kilo entrypoint (thin)
...
- Workspace-generated runtime artifacts exist for `claude` and `codex`
  (via `sync-workspace.sh`).
- Repo-local runtime entrypoint bootstrap exists for all six runtimes:
  `claude` (`CLAUDE.md`), `codex` (`.codex/config.toml` config with
  `AGENTS.md` instructions), `gemini` (`GEMINI.md`),
  `cursor` (`.cursor/rules/governance.mdc`), `copilot`
  (`.github/copilot-instructions.md`), and `kilo`
  (`.kilo/rules/governance.md`) — all generated by `bootstrap-repo.sh`.
