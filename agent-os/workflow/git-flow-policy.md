...
## Workflow Content Cleanliness

The hard prohibition on traces attributable to generative AI or code
generation is defined in `AGENTS.md`. Within the PR-only Git Flow workflow,
that rule applies to all human-facing workflow surfaces:
...
Before push, PR creation/update, or PR merge, agents MUST inspect outgoing
commit messages and proposed PR text for AI/codegen attribution markers and
remove them before continuing.

Common forbidden examples include `Co-Authored-By:`,
`Co-authored-by:`, `Generated by`, and `Generated with` when the attribution
target is an AI model, vendor, or code-generation tool.

This workflow rule does not ban legitimate human co-author trailers.
